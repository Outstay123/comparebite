          ...
          <div className="flex items-center justify-between flex-wrap gap-4">
            ...
            <div className="flex items-center gap-2 bg-blue-50 border border-blue-200 rounded-lg px-3 py-1.5">
              ...
              <span className="text-sm text-blue-800">
                Analyzing <strong>{sellerProducts.length}</strong> products
              </span>
            </div>
          </div>
        </div>
      ...
      <div className="max-w-7xl mx-auto px-4 py-8">
            ...
            {tabs.map((tab) => {
              ...
              return (
                <button
                  ...
                  className={`flex items-center gap-2 px-4 py-3 rounded-xl border-2 font-medium transition-all duration-200 ${getTabStyles(tab.id, tab.color)}`}
                >
        ...
        <div className="space-y-6">
          ...
          {activeTab === 'opportunity' && insights.topOpportunity && (
            <div className="animate-in fade-in slide-in-from-bottom-4 duration-300">
              <Card className="overflow-hidden border-orange-300">
                <CardHeader className="bg-gradient-to-r from-orange-500 to-red-500 text-white py-6">
                  <div className="flex items-center gap-3">
                    <Flame className="w-8 h-8" />
                    <div>
                      <h2 className="text-2xl font-bold">Top Opportunity Product</h2>
                      <p className="text-orange-100">This product has the biggest potential to improve revenue</p>
                    </div>
                  </div>
                </CardHeader>
                <CardBody className="p-6">
                  <div className="grid md:grid-cols-2 gap-6">
                    <div>
                      <h3 className="text-2xl font-bold text-gray-900 mb-2">{insights.topOpportunity.product.name}</h3>
                      <p className="text-gray-500 mb-4">{insights.topOpportunity.product.seller_name}</p>
                      <div className="space-y-3">
                        <div className="flex justify-between p-3 bg-gray-50 rounded-lg">
                          <span className="text-gray-600">Current Profit</span>
                          <span className="font-bold text-gray-900">{formatCurrency(insights.topOpportunity.analysis.profitPerSale)} per sale</span>
                        </div>
                        <div className="flex justify-between p-3 bg-gray-50 rounded-lg">
                          <span className="text-gray-600">Monthly Profit</span>
                          <span className="font-bold text-green-600">{formatCurrency(insights.topOpportunity.analysis.monthlyProfit)}</span>
                        </div>
                        <div className="flex justify-between p-3 bg-gray-50 rounded-lg">
                          <span className="text-gray-600">Value Score</span>
                          <span className="font-bold text-gray-900">{insights.topOpportunity.analysis.valueScore}/100</span>
                        </div>
                      </div>
                    </div>
                    <div className="space-y-4">
                      <div className="bg-red-50 border border-red-200 rounded-xl p-4">
                        <h4 className="font-bold text-red-800 mb-2">Main Issue</h4>
                        <p className="text-red-700">{insights.topOpportunity.weaknessReason}</p>
                      </div>
                      <div className="bg-green-50 border border-green-200 rounded-xl p-4">
                        <h4 className="font-bold text-green-800 mb-2">Opportunity</h4>
                      ...
                      <div className="bg-blue-50 border border-blue-200 rounded-xl p-4">
                        <h4 className="font-bold text-blue-800 mb-2">Recommended Action</h4>
                        <p className="text-blue-700">{insights.topOpportunity.recommendedAction}</p>
                      </div>
                    </div>
                  </div>
                </CardBody>
              ...
              <div className="grid grid-cols-1 md:grid-cols-4 gap-4">
                <Card className="border-green-200">
                  <CardBody className="p-4 text-center bg-green-50">
                    <p className="text-xs text-green-600 mb-1">Total Estimated Profit</p>
                    <p className="text-2xl font-bold text-green-700">{formatCurrencyShort(insights.menuProfit.totalEstimatedProfit)}</p>
                    <p className="text-xs text-green-500">/month</p>
                  </CardBody>
                </Card>
                <Card>
                  <CardBody className="p-4 text-center">
                    <p className="text-xs text-gray-500 mb-1">Total Estimated Revenue</p>
                    <p className="text-2xl font-bold text-gray-900">{formatCurrencyShort(insights.menuProfit.totalEstimatedRevenue)}</p>
                  </CardBody>
                </Card>
                <Card>
                  <CardBody className="p-4 text-center">
                    <p className="text-xs text-gray-500 mb-1">Avg Profit/Product</p>
                    <p className="text-2xl font-bold text-gray-700">{formatCurrency(insights.menuProfit.averageProfitPerProduct)}</p>
                  </CardBody>
                </Card>
                <Card className="border-blue-200">
                  <CardBody className="p-4 text-center bg-blue-50">
                    <p className="text-xs text-blue-600 mb-1">Best Profit Product</p>
                    <p className="text-lg font-bold text-blue-700 truncate">{insights.menuProfit.bestProfitProduct?.product.name || 'N/A'}</p>
                  </CardBody>
                </Card>
              </div>
            ...
            <div className="animate-in fade-in slide-in-from-bottom-4 duration-300 space-y-6">
              {insights.lowProfitItems.length > 0 ? (
                  ...
                  <Card className="border-red-200">
                    <CardBody className="p-6 bg-red-50">
                      <div className="flex items-center gap-4">
                        ...
                        <div>
                          <h3 className="font-bold text-red-800">{insights.lowProfitItems.length} Low Profit Items Found</h3>
                          <p className="text-red-600">Potential monthly loss: {formatCurrency(insights.optimizationImpact.currentTotalLoss)}</p>
                          <p className="text-sm text-red-500">Realistic recovery potential: {formatCurrency(insights.optimizationImpact.potentialRecovery)}/month</p>
                        </div>
                      </div>
                    </CardBody>
                  </Card>
                        ...
                        <CardBody className="p-4">
                          <div className="flex items-center justify-between">
                            <div>
                              ...
                              <p className="text-sm text-gray-500">{item.product.seller_name}</p>
                            </div>
                            <Badge variant="error">Profit: {formatCurrency(item.profitPerSale)}</Badge>
                          </div>
                          <div className="mt-3 p-3 bg-yellow-50 rounded-lg border border-yellow-200">
                            <p className="text-sm text-yellow-800"><strong>Recommendation:</strong> {item.recommendation}</p>
                          </div>
                        </CardBody>
                      </Card>
                    ))}
                ...
                <Card className="text-center p-12">
                  ...
                  <h3 className="text-lg font-bold text-gray-900 mb-2">All Products Have Good Profit Margins</h3>
                  <p className="text-gray-500">No low-profit items found. Your menu is well-optimized!</p>
                </Card>
              )}
            </div>
          )}
              ...
              {insights.overpricedRisks.length > 0 ? (
                <>
                  <Card className="border-yellow-200">
                    <CardBody className="p-6 bg-yellow-50">
                      <div className="flex items-center gap-4">
                        <AlertTriangle className="w-8 h-8 text-yellow-600" />
                        <div>
                          <h3 className="font-bold text-yellow-800">{insights.overpricedRisks.length} Overpriced Risk Items</h3>
                          <p className="text-yellow-600">These products are significantly above category average</p>
                        </div>
                      </div>
                    </CardBody>
                  </Card>
                      ...
                      <Card key={item.product.id} className={`border-l-4 ${item.vsCategoryAvg > 30 ? 'border-l-red-500' : item.vsCategoryAvg > 20 ? 'border-l-yellow-500' : 'border-l-orange-500'}`}>
                        <CardBody className="p-4">
                          <div className="flex items-start justify-between">
                            <div>
                              <h4 className="font-bold text-gray-900">{item.product.name}</h4>
                              <p className="text-sm text-gray-500">{formatCurrency(item.sellingPrice)} vs avg {formatCurrency(item.categoryAvgPrice)}</p>
                            </div>
                            <Badge variant={item.vsCategoryAvg > 30 ? 'error' : item.vsCategoryAvg > 20 ? 'warning' : 'default'}>
                          ...
                          <div className="mt-3 grid grid-cols-2 gap-4">
                            <div className="p-3 bg-red-50 rounded-lg">
                              ...
                              <p className="font-bold text-red-700">{item.customerLossRisk}</p>
                            </div>
                            <div className="p-3 bg-green-50 rounded-lg">
                              <p className="text-xs text-green-600">Suggested Price</p>
                              <p className="font-bold text-green-700">{formatCurrency(item.suggestedPrice)}</p>
                            </div>
                          </div>
                        </CardBody>
                      </Card>
                ...
                <Card className="text-center p-12">
                  <CheckCircle className="w-16 h-16 text-green-500 mx-auto mb-4" />
                  <h3 className="text-lg font-bold text-gray-900 mb-2">No Overpricing Risks</h3>
                  <p className="text-gray-500">All your products are competitively priced within market range.</p>
                </Card>
              )}
            </div>
          )}
          ...
          {activeTab === 'simulation' && (
            <div className="animate-in fade-in slide-in-from-bottom-4 duration-300">
              <Card>
                <CardHeader className="border-b border-gray-100 py-4 bg-blue-50">
                  <h3 className="font-bold text-gray-900 flex items-center gap-2">
                    ...
                    <>
                      ...
                      <div className="grid grid-cols-2 gap-4">
                        <div className="p-4 bg-gray-100 rounded-lg">
                          ...
                          <p className="text-sm text-gray-600">Price: {formatCurrency(selectedProduct.price)}</p>
                          <p className="text-sm text-gray-600">Value Score: {selectedProduct.best_value_score || 'N/A'}</p>
                          <p className="text-sm text-gray-600">Est. Profit: {formatCurrency((selectedProduct.price * 0.4))}</p>
                        </div>
                        <div className="p-4 bg-blue-50 rounded-lg border border-blue-200">
                          <p className="text-sm font-medium text-blue-700 mb-2">If Price = {formatCurrency(simulatedPrice)}</p>
              ...
              <div className="grid grid-cols-1 md:grid-cols-3 gap-4">
                <Card>
                  <CardBody className="p-4 text-center">
                    ...
                    <p className="text-3xl font-bold text-purple-600">{insights.menuBalance.diversity}</p>
                ...
                <Card>
                  <CardBody className="p-4 text-center">
                    ...
                    <p className="text-lg font-bold text-gray-900 capitalize">{insights.menuBalance.categoryDistribution[0]?.category.replace(/_/g, ' ') || 'N/A'}</p>
                  ...
                  <div className="space-y-3">
                    {insights.menuBalance.categoryDistribution.map((cat) => (
                      <div key={cat.category} className="flex items-center gap-3">
                        <div className="w-24 text-sm capitalize">{cat.category.replace(/_/g, ' ')}</div>
                        ...
                        <div className="w-16 text-right text-sm font-medium">{cat.count} items</div>
                        <div className="w-12 text-right text-sm text-gray-500">{cat.percentage.toFixed(0)}%</div>
                      </div>
                    ))}
                  </div>
                </CardBody>
